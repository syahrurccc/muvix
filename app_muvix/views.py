...
def fetch_movies(request, type):
    ...
    if type == "playing":
        ...
        return JsonResponse(
            {
                "error": "Invalid route. You will be redirected in a moment.",
                "redirect": "/",
            },
            status=404,
        )
...
def movie_api(request, movie_id):
    ...
    try:
        ...
        return JsonResponse(
            {
                "error": "Movie doesn't exists. You will be redirected in a moment.",
                "redirect": "/",
            },
            status=404,
        )
...
def fetch_shows(request, movie_id):
    ...
    shows = (
        movie.shows.filter(date=date_obj)
...
def render_seats(request, show_id):
    ...
    if not request.user.is_authenticated:
        return JsonResponse(
            {"error": "Please log in first.", "redirect": "/login"}, status=401
        )
    ...
    try:
        ...
        reserved_ids = list(
            ReservedSeat.objects.filter(show_id=show_id).values_list(
                "seat_id", flat=True
            )
        )
        ...
        return JsonResponse(
            {
                "error": "Show doesn't exists. You will be redirected in a moment.",
                "redirect": "/",
            },
            status=404,
        )
    ...
    return JsonResponse(
        {"seatMap": seat_map, "seatIds": seat_ids, "reservedIds": reserved_ids}
    )
...
def reserve_seats(request, show_id):
    ...
    if not (card_number and expiry_date and cvv and name):
        return JsonResponse(
            {"error": "One or more required fields are empty."}, status=400
        )
    elif not seat_ids:
    ...
    with transaction.atomic():
        ...
        rs = [
            ReservedSeat(reservation=reservation, show=show, seat=seat)
            for seat in seats
        ]
        try:
            ...
            return JsonResponse(
                {"error": "Unable to book seats, please try again."}, status=400
            )

    return JsonResponse(
        {"message": "Successfully book seats, you will be redirected in a moment."},
        status=200,
    )
    ...
    try:
        ...
        reservations = (
            ...
            .prefetch_related(
                Prefetch(
                    "reserved_seats",
                    queryset=ReservedSeat.objects.select_related("seat"),
                )
            )
            .order_by("-created_at")
        )
...
def register(request):
    if request.method == "POST":
        ...
        if password != confirmation:
            return render(
                request, "muvix/register.html", {"message": "Passwords must match."}
            )
        ...
        try:
            ...
            return render(
                request, "muvix/register.html", {"message": "Username already taken."}
            )
        login(request, user)
        return HttpResponseRedirect(reverse("index"))
    else:
